...
export default function BothPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      api.getExportCount({ channel_journey: CHANNEL }),
    ]).then(([f, a, c, ec]) => {
      if (cancelled) return
      setFreq(f || [])
      setAov(a || [])
  ...
  const downloadAll = () =>
    window.open(api.getExportUrl({ channel_journey: CHANNEL }), '_blank')

  return (
    <div style={{ padding:'32px 36px', maxWidth:1400, margin:'0 auto' }}>
      <PageHeader
        title="Both Channel Customers"
        subtitle="Customers who shop online and in-store — migration and omnichannel behaviour"
        action={
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
        ...
        <div className="card" style={{ padding:24 }}>
          <div style={{ fontSize:14, fontWeight:600, marginBottom:4 }}>AOV — New vs Repeat{month ? ` · ${monthLabel}` : ''}</div>
          <div style={{ fontSize:12, color:'var(--text-muted)', marginBottom:16 }}>
            Average order value split by EBO (in-store) and Online channels
          </div>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={aov}>
                ...
                <Tooltip contentStyle={{background:'var(--bg-card)',border:'1px solid var(--border)',
                  borderRadius:8,fontSize:12}} formatter={v=>[`₹${v}`]}/>
                <Bar dataKey="avg_ebo_aov"    name="EBO AOV"    fill="var(--amber)"  radius={[4,4,0,0]}>
                  <LabelList dataKey="avg_ebo_aov"    position="top" formatter={v=>`₹${v}`} style={{fontSize:10,fill:'var(--text-muted)'}}/>
                </Bar>
                <Bar dataKey="avg_online_aov" name="Online AOV" fill="var(--accent)" radius={[4,4,0,0]}>
                  <LabelList dataKey="avg_online_aov" position="top" formatter={v=>`₹${v}`} style={{fontSize:10,fill:'var(--text-muted)'}}/>
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          )}
          <div style={{ display:'flex', gap:12, marginTop:8 }}>
            {[['EBO AOV','var(--amber)'],['Online AOV','var(--accent)']].map(([l,c]) => (
        ...
        <div className="card" style={{ padding:24 }}>
          ...
          <div style={{ fontSize:12, color:'var(--text-muted)', marginBottom:16 }}>
            How many times omnichannel customers have ordered across both channels
          </div>
          {loading ? <CardSkeleton rows={4}/> : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={freq} layout="vertical">
                ...
                <Bar dataKey="customers" name="Customers" fill="#8b5cf6" radius={[0,4,4,0]}>
